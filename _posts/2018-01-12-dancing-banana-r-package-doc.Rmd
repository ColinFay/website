...
A package documentation is composed of .Rd files, which lives (when developping) in the `/man` folder of your source package. They are made of LaTeX, but nowadays we rarely write LaTeX by hand, but rely on {roxygen2} for doing the heavy lifting for package documentation. But still, you can write LaTeX code into the roxygen comments. That's the trick I'm gonna used, borrowed to Jeroen {writexl}'s code. 
...
With that, the HTML/JS code just pops up at the bottom of the documentation, when doing `?myfun`. 
...
But I guess it's still better to get [rickrolled](https://www.youtube.com/watch?v=dQw4w9WgXcQ), so here's the code to do that (and if you have mercy for your users, turn the autoplay off by changing to `autoplay=0`  ;) ):
