...
I've been working lately on a new package called {trycatchthis}, which is an attempt at making try catch and condition handling in R a little bit friendlier. If you have some time to spare, I'll be glad if you can give me some feedbacks about it: positive, negative, PR, improvement suggestions [https://github.com/ColinFay/trycatchthis](https://github.com/ColinFay/trycatchthis)... Feel free! 
...
These are also what allows {trycatchthis} to be flexible when building trycatch calls and condition handlers. 
...
So if you've followed along, you now know that I can use mappers here as arguments of a `tryCatch` call. This is exactly what `try_catch`, from {trycatchthis}, does. 

Here's a shorten example (and slightly different from the one in {trycatchthis} as `try_that` is built with {rlang} `lang`) : 
...
You can have a look at what {trycatchthis} does on the [GitHub repo](https://github.com/ColinFay/trycatchthis). 
