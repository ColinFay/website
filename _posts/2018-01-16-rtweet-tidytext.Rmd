...
But everything is now back in order with the new {tidytext} version (at least for the text column). Let's be crazy and install the latest version from Github : 
...
This no works because {tidytext} no longer checks if all the columns are atomic. Yet we got an issue if we move to the hashtags column : 
...
So here's the workaround with {purrr} and {dplyr}: 

```{r eval = FALSE}
library(purrr)
rtweets06 %>%
  mutate(hashtags = map(hashtags, ~ simplify(.x) %>% paste(collapse = " ")) %>% as_vector) %>%
  tidytext::unnest_tokens(word, hashtag) %>% 
  select(screen_name, word) %>%
  slice(1:5)
```
