...
```{r}collection_complete &lt;- jsonlite::fromJSON(txt = "http://colinfay.me/data/collection_complete.json", simplifyDataFrame = TRUE)```
...
```{r}collec_url &lt;- httr::GET(paste0("https://api.discogs.com/users/", user, "/collection/folders/", content[[1]]$id, "/releases?page=1&amp;per_page=100"))
...
```{r}ggplot(as.data.frame(head(sort(table(collection$artist_name), decreasing = TRUE), 10)), aes(x = reorder(Var1, Freq), y = Freq)) + 
  geom_bar(stat = "identity", fill = "#B79477") + 
  coord_flip() + 
  xlab("Artistes") +
  ylab("Fréquence") +
...
```{r}ggplot(dplyr::filter(collection, year != 0), aes(x = year)) + 
...
#### Hello, it's me again
&nbsp;
```{r}collection_2 &lt;- lapply(as.list(collection$release_id), function(obj){
...
```{r}ggplot(as.data.frame(head(sort(table(collection_2$genre), decreasing = TRUE), 10)), aes(x = reorder(Var1, Freq), y = Freq)) + 
  geom_bar(stat = "identity", fill = "#B79477") + 
  coord_flip() + 
  xlab("Genre") +
  ylab("Fréquence") +
...
```{r}ggplot(as.data.frame(head(sort(table(collection_2$country), decreasing = TRUE), 10)), aes(x = reorder(Var1, Freq), y = Freq)) + 
  geom_bar(stat = "identity", fill = "#B79477") + 
  coord_flip() + 
  xlab("Pays d'origine") +
  ylab("Fréquence") +
...
```{r}ggplot(collection_2, aes(x = average_note)) + 
...
#### Money Money Money
```{r}ggplot(collection_2, aes(x = lowest_price)) + 
  geom_histogram(fill = "#B79477") +
...
### Let’s finish!
```{r}collection_complete &lt;- merge(collection, collection_2, by = c("release_id","label", "year", "title", "artist_name"))```
#### Prix des vinyles en fonction des “want”
```{r}lm_want &lt;- lm(formula = lowest_price ~ want, data = collection_complete)
summary(lm_want)```
<pre><code>##Residuals:
...
```{r}ggplot(collection_complete, aes(x = lowest_price, y = want)) + 
...
```{r}lm_note &lt;- lm(formula = lowest_price ~ average_note, data = collection_complete)
...
```{r}ggplot(collection_complete, aes(x = lowest_price, y = average_note)) + 
  geom_point(size = 3, color = "#B79477") +
  xlab("Prix le plus bas") +
  ylab("Note moyenne") +
  ylim(c(0,5)) +
