...
Nop, still quite the same. Ok, maybe that's because their name are composed of repeated letters (remember... Xaro Xhoan Daxos)? 
...
# Here, a result of 1 means every letters are different. The closer they get to zero, the more repeated they are. 
...
Ok, just a little less difference that the others. Now, let's try some string dist approach.

## String dist 

### Chosing a string distance algorithm  

My main point while watching GoT is that "everybody names sound the same". Like, why on earth should you name your character "Bran" and "Bronn", "Tyron and Tywinn", "Tommen" and "Tormund", and there is literally a "Robb", a "Robert" and a "Robin". 

Can we see it with stringdistance? My first intuition is that string distance should be shorter in Game of Thrones than in other shows. Let's try several methods to find out. 

### Levenshtein distance

The Levenshtein distance (method='lv') counts the number of deletions, insertions and substitutions necessary to turn b into a. For example, `stringdist("Colin", "Colis", method = "lv")` returns 1.
...
Well, Jaro is neither corrobarating my initial idea... it seems like my short memory and me are not supported by data :) 
